	...
	DialogInterface.OnDismissListener, MenuPresenter.Callback
{
    ...
    if (event.getAction() == KeyEvent.ACTION_DOWN && event.getRepeatCount() == 0) {
      return trackKeyEvent(dialog);
    } else if (event.getAction() == KeyEvent.ACTION_UP && !event.isCanceled()) {
      return handleKeyEventUp(dialog, event);
    } 

    return false;
  }

  private boolean trackKeyEvent(DialogInterface dialog) {
  ...
  private boolean handleKeyEventUp(DialogInterface dialog, KeyEvent event) {
    Window win = mDialog.getWindow();
    if (win != null) {
      View decor = win.getDecorView();
      if (decor != null) {
