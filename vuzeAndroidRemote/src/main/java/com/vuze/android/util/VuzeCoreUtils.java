...
public class VuzeCoreUtils
{
	...
	private static VuzeServiceInit createVuzeServiceInit() {
        ...
        addCoreStateListeners(mapListeners);

        return new VuzeServiceInitImpl(VuzeRemoteApp.getContext(), mapListeners);
    }

    private static void addCoreStateListeners(Map<String, Runnable> mapListeners) {
        mapListeners.put("onAddedListener", createOnAddedListener());
        mapListeners.put("onCoreStarted", createOnCoreStartedListener());
        mapListeners.put("onCoreStopping", createOnCoreStoppingListener());
        mapListeners.put("onCoreRestarting", createOnCoreRestartingListener());
    }

    private static Runnable createOnAddedListener() {
        return new RunnableWithObject() {
            @Override
            public void run() {
                if (AndroidUtils.DEBUG) {
                ...
                handleCoreState((String) object);
            }

            private void handleCoreState(String state) {
    ...
    private static Runnable createOnCoreStartedListener() {
        return new Runnable() {
            @Override
            public void run() {
                if (AndroidUtils.DEBUG) {
    ...
    private static Runnable createOnCoreStoppingListener() {
        return new Runnable() {
            @Override
            public void run() {
    ...
    private static Runnable createOnCoreRestartingListener() {
        return new Runnable() {
            @Override
            public void run() {
                // Core Restarting
