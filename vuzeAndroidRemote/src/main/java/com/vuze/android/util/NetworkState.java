...
public class NetworkState
{
	...
	*/
	...
	private static String getIpAddress_Old(String startsWith) {
          ...
          try {
               Enumeration<NetworkInterface> networkInterfaces = getNetworkInterfaces();
               ipAddress = getIpAddressFromInterfaces(networkInterfaces, startsWith);
          } catch (SocketException ex) {
     ...
     private static String getIpAddressFromInterfaces(Enumeration<NetworkInterface> networkInterfaces, String startsWith) throws SocketException {
          String ipAddress = "127.0.0.1";
          for (Enumeration<NetworkInterface> en = networkInterfaces; en.hasMoreElements();) {
               NetworkInterface intf = en.nextElement();
               if (intf.getName().startsWith("usb")) {
                    logIgnoredInterface(intf, startsWith);
                    continue;
               }
               ipAddress = getIpAddressFromInterface(intf, startsWith);
               if (startsWith != null && ipAddress != null && intf.getName().startsWith(startsWith)) {
                    return ipAddress;
               }
          }
          return ipAddress;
     ...
     private static String getIpAddressFromInterface(NetworkInterface intf, String startsWith) throws SocketException {
          String ipAddress = null;
          for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements();) {
               InetAddress inetAddress = enumIpAddr.nextElement();
               if (!inetAddress.isLoopbackAddress() && (inetAddress instanceof Inet4Address)) {
                    ipAddress = inetAddress.getHostAddress();
                    logIpAddress(intf, inetAddress, ipAddress);
                    if (startsWith != null && intf.getName().startsWith(startsWith)) {
                         return ipAddress;
                    }
               }
     ...
     private static void logIgnoredInterface(NetworkInterface intf, String startsWith) {
          if (AndroidUtils.DEBUG) {
               if (startsWith == null || intf.getName().startsWith(startsWith)) {
     ...
     private static void logIpAddress(NetworkInterface intf, InetAddress inetAddress, String ipAddress) {
          if (AndroidUtils.DEBUG) {
