	...
	ActionModeBeingReplacedListener, NetworkStateListener
{
	...
	public void rpcTorrentListReceived(String callID, List<?> addedTorrentMaps,
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (removedTorrentIDs != null && isTorrentRemoved(removedTorrentIDs)) {
	...
	private boolean isTorrentRemoved(List<?> removedTorrentIDs) {
		for (Object removedItem : removedTorrentIDs) {
			if (removedItem instanceof Number && torrentID == ((Number) removedItem).longValue()) {
				return true;
			}
		}
		return false;
