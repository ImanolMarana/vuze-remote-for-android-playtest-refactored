...
public class TorrentUtils
{
	...
	public static String getSaveLocation(Session session,
			Map<?, ?> mapTorrent) {
		String saveLocation = getInitialSaveLocation(session, mapTorrent);
		...
		if (listFiles == null) {
			...
			saveLocation = adjustSaveLocationForSingleFile(mapTorrent, saveLocation);
		} else if (listFiles.size() == 1) {
			saveLocation = adjustSaveLocationForSingleFileFromList(listFiles,
					saveLocation);
		}
	...
	private static String getInitialSaveLocation(Session session, Map<?, ?> mapTorrent) {
		String saveLocation = MapUtils.getMapString(mapTorrent,
				TransmissionVars.FIELD_TORRENT_DOWNLOAD_DIR, null);

		if (saveLocation == null) {
			saveLocation = getDefaultSaveLocation(session);
		}
		return saveLocation;
	}

	private static String getDefaultSaveLocation(Session session) {
		String saveLocation;
		if (session == null) {
			saveLocation = "dunno";
		} else {
	...
	private static String adjustSaveLocationForSingleFile(Map<?, ?> mapTorrent,
			String saveLocation) {
	...
	private static String adjustSaveLocationForSingleFileFromList(
			List<?> listFiles, String saveLocation) {
