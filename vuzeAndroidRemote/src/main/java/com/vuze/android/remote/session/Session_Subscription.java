...
public class Session_Subscription
{
	...
	public void removeSubscription(@NonNull Activity activity,
		...
		String message = getRemovalConfirmationMessage(resources, subscriptionIDs);
		@StringRes int titleID = getRemovalConfirmationTitle(subscriptionIDs);

		DialogInterface.OnClickListener onDeleteClicked = getOnDeleteClickListener(
				subscriptionIDs, l);
	...
	private String getRemovalConfirmationMessage(Resources resources, String[] subscriptionIDs) {
		if (subscriptionIDs.length == 1) {
			Map<?, ?> subscription = session.subscription.getSubscription(
					subscriptionIDs[0]);
			String name = MapUtils.getMapString(subscription, "name", null);
			return resources.getString(R.string.subscription_remove_text,
					TextUtils.htmlEncode(name));
		} else {
			return resources.getString(R.string.subscriptions_remove_text,
					"" + subscriptionIDs.length);
		}
	}

	@StringRes
	private int getRemovalConfirmationTitle(String[] subscriptionIDs) {
		return subscriptionIDs.length == 1 ? R.string.subscription_remove_title
				: R.string.subscriptions_remove_title;
	}

	private DialogInterface.OnClickListener getOnDeleteClickListener(
			final String[] subscriptionIDs, final SubscriptionsRemovedListener l) {
			...
			public void onClick(DialogInterface dialog, int which) {
				session.transmissionRPC.removeSubscriptions(subscriptionIDs,
						new ReplyMapReceivedListener() {
							@Override
							public void rpcError(String id, Exception e) {
								handleSubscriptionRemovalError(l, e, null);
							}

							@Override
							public void rpcFailure(String id, String message) {
								handleSubscriptionRemovalError(l, null, message);
							}

							@Override
							public void rpcSuccess(String id, Map<?, ?> optionalMap) {
								handleSubscriptionRemovalSuccess(l, optionalMap);
							}
						});
			}
		};
	...
	private void handleSubscriptionRemovalError(SubscriptionsRemovedListener l,
			Exception e, String message) {
		if (l != null) {
			l.subscriptionsRemovalException(e, message);
		}
	}

	private void handleSubscriptionRemovalSuccess(SubscriptionsRemovedListener l,
			Map<?, ?> optionalMap) {
		refreshList();
		if (l == null) {
			return;
