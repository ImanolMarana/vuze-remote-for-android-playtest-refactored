...
public class Session_Tag
{
    ...
    synchronized (session.mLock) {
      LongSparseArray mapNewTags = buildNewTagMap(tagList);
  ...
  private LongSparseArray buildNewTagMap(List<?> tagList) {
    int numUserCategories = 0;
    long uidUncat = -1;
    LongSparseArray mapNewTags = new LongSparseArray<>(tagList.size());
    for (Object tag : tagList) {
      if (tag instanceof Map) {
        processTag(mapNewTags, (Map<?, ?>) tag, ref -> {
          numUserCategories += ref.numUserCategories;
          uidUncat = ref.uidUncat;
        });
      }
    }
  ...
  private void processTag(LongSparseArray mapNewTags, Map<?, ?> mapNewTag, TagProcessor processor) {
    Long uid = MapUtils.getMapLong(mapNewTag, "uid", 0);
    Map mapOldTag = mapTags == null ? null : mapTags.get(uid);
    if (mapNewTag.containsKey("name")) {
      updateTagName(mapNewTags, uid, mapNewTag, mapOldTag);
    } else {
      updateTagCount(mapNewTags, uid, mapNewTag, mapOldTag);
    }
  
    int type = MapUtils.getMapInt(mapNewTag, "type", 0);
    //category
    if (type == 1) {
      updateTagCategory(mapNewTag, uid, processor);
    }
  }
  
  private void updateTagCategory(Map<?, ?> mapNewTag, Long uid, TagProcessor processor) {
    // USER=0,ALL=1,UNCAT=2
    int catType = MapUtils.getMapInt(mapNewTag,
        TransmissionVars.FIELD_TAG_CATEGORY_TYPE, -1);
    if (catType == 0) {
      processor.numUserCategories++;
    } else if (catType == 1) {
      tagAllUID = uid;
    } else if (catType == 2) {
      processor.uidUncat = uid;
    }
  }
  
  private void updateTagCount(LongSparseArray mapNewTags, Long uid, Map<?, ?> mapNewTag, Map mapOldTag) {
  ...
  private void updateTagName(LongSparseArray mapNewTags, Long uid, Map<?, ?> mapNewTag, Map mapOldTag) {
    if (mapOldTag == null) {
