...

{
	...
	private boolean handleIntent(Intent intent,
		...
		if (handleDataSchemeIntent(intent)) {
			return true;
		}
		
		return handleLaunchIntent(intent, savedInstanceState, forceProfileListOpen);
	}

	private boolean handleLaunchIntent(Intent intent, Bundle savedInstanceState, boolean forceProfileListOpen) {
	...
	private boolean handleDataSchemeIntent(Intent intent) {
		...
		if (data != null) {
			try {
				...
				if (handleVuzeScheme(intent, data, scheme, host)) {
					return true;
				}

				if (handleRemoteVuzeComScheme(intent, data, host)) {
					return true;
				}
	...
	private boolean handleRemoteVuzeComScheme(Intent intent, Uri data, String host) {
	...
	private boolean handleVuzeScheme(Intent intent, Uri data, String scheme, String host) {
		// check for vuze://remote//*
		if ("vuze".equals(scheme) && "remote".equals(host) && data.getPath() != null
