...
public class AppPreferences
{
	...
	static boolean importPrefs_withPerms(Activity activity, Uri uri) {
		...
		if ("file".equals(scheme) || "content".equals(scheme)) {
			return handleFileContentSchemeImport(activity, uri);
		} else {
	...
	private static InputStream getInputStreamForUri(Activity activity, Uri uri) throws FileNotFoundException {
		String realPath = PaulBurkeFileUtils.getPath(activity, uri);
		if (realPath != null) {
			String meh = realPath.startsWith("/") ? "file://" + realPath
					: realPath;
			return activity.getContentResolver().openInputStream(
					Uri.parse(meh));
		}
		return null;
	}

	private static boolean handleFileContentSchemeImport(Activity activity, Uri uri) {
		try {
			InputStream stream = null;
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
				stream = getInputStreamForUri(activity, uri);
			}
			if (stream == null) {
