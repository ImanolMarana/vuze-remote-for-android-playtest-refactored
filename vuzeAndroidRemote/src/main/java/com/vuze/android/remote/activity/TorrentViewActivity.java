	...
	SessionListener, NetworkStateListener, TorrentListRefreshingListener
{
	...
	public void onTorrentSelectedListener(TorrentListFragment torrentListFragment,
		...
		if (DEBUG) {
			Log.d(TAG,
					"onTorrentSelectedListener: " + Arrays.toString(ids) + ";multi?"
							+ inMultiMode + "; hasMagnet:" + hasMagnetTorrent(ids) + "; "
							+ getSupportFragmentManager().findFragmentById(
									R.id.frag_torrent_details) + " via " + AndroidUtils.getCompressedStackTrace());
		}

		if (hasMagnetTorrent(ids)) {
			hideTorrentDetails();
			return;
		}
		...
		if (detailFrag != null && fragmentView != null) {
			...
			handleTwoPaneLayout(ids, detailFrag, fragmentView);
		} else if (ids != null && ids.length == 1 && !inMultiMode) {
			startTorrentDetailsActivity(torrentListFragment, ids);
		}
	}

	private boolean hasMagnetTorrent(long[] ids) {
		if (ids == null) {
			return false;
		}
	...
	private void hideTorrentDetails() {
		TorrentDetailsFragment detailFrag = (TorrentDetailsFragment) getSupportFragmentManager().findFragmentById(
				R.id.frag_torrent_details);
		View fragmentView = findViewById(R.id.frag_details_container);
		if (detailFrag != null && fragmentView != null) {
	...
	private void handleTwoPaneLayout(long[] ids, TorrentDetailsFragment detailFrag, View fragmentView) {
	...
	private void startTorrentDetailsActivity(TorrentListFragment torrentListFragment, long[] ids) {
		Intent intent = new Intent(Intent.ACTION_VIEW, null, this,
