...

{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        setupSwipeRefresh(fragView);

        setupRecyclerView(fragView);
    ...
    private void setupSwipeRefresh(View fragView) {
        ...
        if (swipeRefresh != null) {
            ...
            updateSwipeRefreshLastUpdated(swipeRefresh);

            swipeRefresh.setOnRefreshListener(
                    new SwipeRefreshLayout.OnRefreshListener() {
                        @Override
                        public void onRefresh() {
                            refreshTorrentList(swipeRefresh);
                        }
                    });
            swipeRefresh.setOnExtraViewVisibilityChange(
                    new SwipeRefreshLayoutExtra.OnExtraViewVisibilityChangeListener() {
                        ...
                        public void onExtraViewVisibilityChange(final View view,
                                                                int visibility) {
                            handleExtraViewVisibilityChange(view, visibility);
                        }

                        private void handleExtraViewVisibilityChange(final View view, int visibility) {
                            ...
                            pullRefreshHandler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    updateLastUpdatedInfo(view);
                                }
                            }, 0);
                        }

                        private void updateLastUpdatedInfo(View view) {
                            if (getActivity() == null) {
                                return;
                            }
    ...
    private void refreshTorrentList(final SwipeRefreshLayoutExtra swipeRefresh) {
        Session session = getSession();
    ...
    private void updateSwipeRefreshLastUpdated(SwipeRefreshLayoutExtra swipeRefresh) {
        LastUpdatedInfo lui = getLastUpdatedString();
        if (lui != null) {
