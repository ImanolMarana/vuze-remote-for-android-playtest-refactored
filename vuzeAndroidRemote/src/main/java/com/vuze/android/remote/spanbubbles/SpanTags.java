...
public class SpanTags
{
	...
	private void setTagBubbles(final SpannableStringBuilder ss, String text,
          ...
          while (true) {
              ...
              base = end + tokenLen;
              processTagSpan(ss, text, start, end, tokenLen, index);
              index++;
          }
      }

      private void processTagSpan(SpannableStringBuilder ss, String text, int start, int end, int tokenLen, final int index) {
          final int fSpanStart = start;
          final int fSpanEnd = end + tokenLen;
          String id = text.substring(start + tokenLen, end);

          Map mapTag = getTagMap(id);

          final String word = MapUtils.getMapString(mapTag, "name", "" + id);
          final Map fMapTag = mapTag;

          final DrawableTag imgDrawable = createDrawableTag(mapTag, word, index);
          setImageSpan(ss, imgDrawable, start, end, tokenLen);

          if (listener != null) {
              setClickableSpan(ss, mapTag, word, index, fSpanStart, fSpanEnd, start, end, tokenLen);
          }
      }

      private Map getTagMap(String id) {
          Map mapTag = null;
          try {
      ...
      private DrawableTag createDrawableTag(final Map mapTag, final String word, final int index) {
          final DrawableTag imgDrawable = new DrawableTag(context, tvTags.getPaint(), word,
              ...
              public int getTagState() {
                  ...
                  return listener.getTagState(index, fMapTag, word);
              }
          ...
          if (listener != null && showIcon) {
              int tagState = listener.getTagState(index, mapTag, word);
      ...
      private void setImageSpan(SpannableStringBuilder ss, DrawableTag imgDrawable, int start, int end, int tokenLen) {
      ...
      private void setClickableSpan(SpannableStringBuilder ss, final Map mapTag, final String word, final int index, final int fSpanStart, final int fSpanEnd, int start, int end, int tokenLen) {
          ClickableSpan clickSpan = new ClickableSpan() {

              @Override
              public void onClick(View widget) {
                  listener.tagClicked(index, mapTag, word);
