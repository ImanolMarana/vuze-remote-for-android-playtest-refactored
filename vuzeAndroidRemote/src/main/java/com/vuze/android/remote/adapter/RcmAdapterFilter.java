	...
	extends LetterFilter<String>
{
    ...
    if (!filterPublishDate(map)) {
      return false;
    }
    if (!filterLastSeen(map)) {
      return false;
    }
    if (!filterMinSeeds(map)) {
      return false;
    }
    return filterMinRank(map);
  }

  private boolean filterSize(Map<?, ?> map) {
  ...
  private boolean filterPublishDate(Map<?, ?> map) {
  ...
  private boolean filterLastSeen(Map<?, ?> map) {
  ...
  private boolean filterMinSeeds(Map<?, ?> map) {
    if (minSeeds > 0) {
  ...
  private boolean filterMinRank(Map<?, ?> map) {
    if (minRank > 0) {
