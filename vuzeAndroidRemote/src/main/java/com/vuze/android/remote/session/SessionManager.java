...
public class SessionManager
{
    ...
    synchronized (mapSessions) {
      Session session = getOrCreateSession(profileID, activity);
  
      if (!profileID.equals(lastUsed)) {
        lastUsed = profileID;
        updateTrackingInfo(session.getRemoteProfile());
      }
  ...
  private static @NonNull Session getOrCreateSession(@NonNull String profileID, @Nullable Activity activity) {
    Session session = mapSessions.get(profileID);
    ...
    if (session == null) {
      session = createSession(profileID, activity);
    } else {
      if (activity != null) {
  ...
  private static @NonNull Session createSession(@NonNull String profileID, @Nullable Activity activity) {
    RemoteProfile remoteProfile = VuzeRemoteApp.getAppPreferences().getRemote(
        profileID);
    if (remoteProfile == null) {
      logErrorForMissingProfile(profileID, activity);
      // UH OH, breaking the @NotNull
      throw new IllegalStateException("Missing RemoteProfile");
    }
    if (AndroidUtils.DEBUG) {
    ...
    Session session = new Session(remoteProfile);
    if (activity != null) {
      session.setCurrentActivity(activity);
    }
  ...
  private static void updateTrackingInfo(RemoteProfile remoteProfile) {
  ...
  private static void logErrorForMissingProfile(@NonNull String profileID, @Nullable Activity activity) {
    if (AndroidUtils.DEBUG) {
  ...
  private static void addSessionChangedListener(String profileID, SessionChangedListener l) {
    synchronized (changedListeners) {
      List<SessionChangedListener> listeners = changedListeners.get(
          profileID);
      if (listeners == null) {
