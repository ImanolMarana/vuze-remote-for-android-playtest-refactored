	...
	extends RestJsonClient
{
	...
	public Map<?, ?> connect(String id, String url, @Nullable Map<?, ?> jsonPost,
		...
		try {
			...
			OkHttpClient localClient = getClientForURI(uri);

			Request.Builder builder = buildRequestBuilder(id, url, headers);

			handleJsonPost(builder, id, jsonPost);

			handleAuthentication(builder, username, password);
			...
			json = processResponse(response, id);
	...
	private OkHttpClient getClientForURI(URI uri) {
		if (client == null) {
	...
	private Request.Builder buildRequestBuilder(String id, String url, Map<String, String> headers) {
	...
	private void handleJsonPost(Request.Builder builder, String id, Map<?, ?> jsonPost) throws IOException {
		if (jsonPost != null) {
	...
	private void handleAuthentication(Request.Builder builder, String username, String password) {
		if (username != null) {
	...
	private Map<?, ?> processResponse(Response response, String id) throws IOException, RPCException {
		...
		try {
			if (USE_STRINGBUILDER) {
				return processResponseUsingStringBuilder(body, id, contentLength);
			} else {
				return processResponseUsingBufferedReader(body, id);
			}

		} catch (Exception pe) {
			return handleProcessResponseException(response, statusCode, sb, br, pe, id);
		} finally {
			body.close();
		}
	}

	private Map<?, ?> processResponseUsingStringBuilder(ResponseBody body, String id, long contentLength) throws IOException {
		StringBuilder sb;
		Reader isr = body.charStream();
		...
		logResponseBody(id, sb);
		return JSONUtils.decodeJSON(sb.toString());
	}

	private Map<?, ?> processResponseUsingBufferedReader(ResponseBody body, String id) throws IOException {
		Reader isr = body.charStream();
		BufferedReader br = new BufferedReader(isr, 8192);
		br.mark(32767);
		Map<?, ?> json = JSONUtils.decodeJSON(br);
		logResponseBody(id, body);
		return json;
	}

	private Map<?, ?> handleProcessResponseException(Response response, int statusCode, StringBuilder sb, BufferedReader br, Exception pe, String id) throws IOException, RPCException {
		if (statusCode == 409) {
		...
		try {
			...
			if (isHtmlResponse(line, contentType)) {
				handleHtmlResponse(response, statusCode, sb, pe, id);
			}
			if (isBencodedResponse(line)) {
				return handleBencodedResponse(line);
			}
	...
	private void logResponseBody(String id, StringBuilder sb) {
		if (AndroidUtils.DEBUG_RPC) {
	...
	private void logResponseBody(String id, ResponseBody body) {
		if (AndroidUtils.DEBUG_RPC) {
	...
	private boolean isHtmlResponse(String line, MediaType contentType) {
		return line.startsWith("<") || line.contains("<html")
				|| (contentType != null
				&& contentType.type().startsWith("text/html"));
	}

	private void handleHtmlResponse(Response response, int statusCode, StringBuilder sb, Exception pe, String id) throws RPCException {
		if (AndroidUtils.DEBUG_RPC) {
			String msg = statusCode + ": " + response.message() + "\n"
					+ pe.getMessage();
	...
	private boolean isBencodedResponse(String line) {
		return line.matches("^d[0-9]+:.*$");
	}

	private Map<?, ?> handleBencodedResponse(String line) {
